...
def test_compute_length():
    ...
    length = wslfp.lfp_amplitude_function.compute_length(cords, cords)
    for arr in range(len(length)):
        assert length[arr] == 0
    # check that flipping the neuron results in the same length
    length1 = wslfp.lfp_amplitude_function.compute_length([[0,2,0]], [[0,0,0]])
    length2 = wslfp.lfp_amplitude_function.compute_length([[0,0,0]], [[0,2,0]])
...
def test_compute_midpoint():
    ...
    midpoint1 = wslfp.lfp_amplitude_function.compute_midpoint([[0,2,0]], [[0,0,0]])
    midpoint2 = wslfp.lfp_amplitude_function.compute_midpoint([[0,0,0]], [[0,2,0]])
    for arr in range(len(midpoint1)):
        assert midpoint1[arr] == midpoint2[arr]
    # check that fucntion can handle negative values
    midpoint = wslfp.lfp_amplitude_function.compute_midpoint([[0,-1,-3]], [[4,8,-10]])
...
def test_compute_distance():
    ...
    distance = wslfp.lfp_amplitude_function.compute_distance([[0,2,0]], [[0,0,0]])
    for arr in range(len(distance)):
        assert distance[arr] <= 0
    # check that distance is positive when electrode is above neuron
    distance1 = wslfp.lfp_amplitude_function.compute_distance([[0,2,0]], [[0,4,0]])
    ...
    distance2 = wslfp.lfp_amplitude_function.compute_distance([[0,0,0]], [[0,4,0]])
    ...
    distance3 = wslfp.lfp_amplitude_function.compute_distance([[0,0,0]], [[0,-4,0]])
...
def test_compute_angle():
    ...
    angle1 = wslfp.lfp_amplitude_function.compute_angle([[0,2,0]], [[0,1,0]])
    angle2 = wslfp.lfp_amplitude_function.compute_angle([[0,2,0]], [[0,4,0]])
    assert angle1 == angle2

def test_compute_amp():
    # check that LFP Amplitude is positive above the midpoint
    amp1 = wslfp.lfp_amplitude_function.compute_amp([[1,1,1]], [[2,2,2]], [[5,5,5]])
    for arr in range(len(amp1)):
        assert amp1[arr] >= 0
    # check that LFP Amplitude is negarive below the midpoint
    amp2 = wslfp.lfp_amplitude_function.compute_amp([[1,1,1]], [[2,2,2]], [[-5,-5,-5]])
    for arr in range(len(amp2)):
        assert amp2[arr] <= 0
    # check that LFP Amplitude is stronger closer to the neuron
    amp3 = wslfp.lfp_amplitude_function.compute_amp([[1,1,1]],[[2,2,2]],[[3,3,3]])
    amp4 = wslfp.lfp_amplitude_function.compute_amp([[1,1,1]],[[2,2,2]],[[6,6,6]])
    for arr in range(len(amp3)):
        assert amp3[arr] > amp4[arr]
    # check that flipping the neuron results in the same LFP
    amp5 = wslfp.lfp_amplitude_function.compute_amp([[1,1,1]],[[2,2,2]],[[3,3,3]])
    amp6 = wslfp.lfp_amplitude_function.compute_amp([[2,2,2]],[[1,1,1]],[[3,3,3]])
...
def test_random_orientation():
    ...
    amp1 = wslfp.lfp_amplitude_function.compute_amp([soma1], [dendrite1], [electrode1])
    amp2 = wslfp.lfp_amplitude_function.compute_amp([soma2], [dendrite2], [electrode2])
    assert amp1 == amp2
...
def test_multiple_inputs():
    ...
    dendrite = np.random.rand(5, 3) * 10
    electrode = np.random.rand(5, 3) * 10
    amp = wslfp.lfp_amplitude_function.compute_amp(soma, dendrite, electrode)
